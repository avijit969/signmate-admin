...
export default function YesNoQuestion({ totalQuestion }) {
    ...
    const fetchQuestions = async (currentPage) => {
        ...
        try {
            const response = await axios({
                method: 'get',
                url: `${import.meta.env.VITE_API_URL}/questions/yes_no?page=${currentPage}&limit=5`,
    ...
    return (
        <div className="flex flex-col gap-4 border-2 border-black rounded-md p-4">
            <h2 className="text-xl font-bold">Yes-No Questions</h2>
            {isLoading ? (
